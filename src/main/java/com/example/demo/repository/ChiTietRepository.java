...
//    void create(@Param("nv_chinh")int nv_chinh,@Param("cong_viec_id")int cong_viec_id,@Param("nhan_vien_id")int nhan_vien_id);
    @Query(value = "select nhan_viens.full_name from nhan_viens inner join chi_tiet on chi_tiet.nhan_vien_id = nhan_viens.mnv where chi_tiet.cong_viec_id = :cong_viec_id and chi_tiet.nv_chinh = 0 and chi_tiet.is_deleted = 0", nativeQuery = true)
